...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isGoogle) {
      return handleGoogleCallback(code, request);
    } else {
      return handleMicrosoftCallback(code, request);
    }
  } catch (err) {
...
async function handleGoogleCallback(code: string, request: NextRequest) {
  // Exchange code for tokens
  const tokenData = await exchangeCodeForTokens(code);

  if (!tokenData.access_token) {
    return NextResponse.redirect(
      new URL("/settings/email?error=google_token_exchange_failed", request.url)
    );
  }
  ...
  if (!profile.email) {
    return NextResponse.redirect(
      new URL("/settings/email?error=google_no_email", request.url)
    );
  }
...
async function handleMicrosoftCallback(code: string, request: NextRequest) {
