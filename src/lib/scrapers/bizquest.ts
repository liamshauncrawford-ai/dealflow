...
const BASE_URL = "https://www.bizquest.com";
...
export class BizQuestScraper extends BaseScraper {
  ...
  buildSearchUrl(filters: ScraperFilters): string {
    // BizQuest uses path-based state filtering plus query params
  ...
  parseSearchResults(
    ...
    // BizQuest uses several possible selectors for listing cards (same parent company as BizBuySell).
    const listingCards = $(
      ".listing, .listingCard, .search-result, [class*='listing']"
    );
    ...
    listingCards.each((_index, element) => {
      try {
        ...
        const linkEl = card
          .find(
            "a.listingLink, a.listing-title, h3 a, h2 a, a[href*='/listing/']"
          )
          .first();
  ...
  async parseDetailPage(html: string, url: string): Promise<RawListing> {
    ...
    // ── Broker info ──
    const brokerSection = $(
      ".broker, .brokerInfo, [class*='broker'], .contactInfo, .listingBroker"
    );
    const brokerName =
      normalizeText(
        brokerSection
  ...
  getNextPageUrl(html: string): string | null {
    ...
    // BizQuest pagination: look for "Next" link or right-arrow pagination
    const nextLink = $(
   ...
   * BizQuest uses various markup patterns: dt/dd pairs, tables, label/value divs.
   */
  ...
  /**
   * Extract a BizQuest listing source ID from the URL.
   * URLs follow the pattern: /listing/detail/XXXXXXX
   */
  private extractSourceId(url: string): string | null {
    // Pattern: /listing/detail/123456 or numeric ID at end of URL path
    const detailMatch = url.match(/\/listing\/detail\/(\d+)/);
