...
export class BizBuySellScraper extends BaseScraper {
  ...
  async parseDetailPage(html: string, url: string): Promise<RawListing> {
    ...
    return {
      sourceId,
      sourceUrl: url,
      title: sanitizeScrapedText(title) || "Untitled Listing",
      askingPrice,
      revenue,
      cashFlow,
      ebitda,
      sde,
      industry: industry ? sanitizeScrapedText(industry) || null : null,
      category: category ? sanitizeScrapedText(category) || null : null,
      city: location.city,
      state: location.state,
      zipCode: location.zipCode,
      description: description ? sanitizeScrapedText(description) || null : null,
      brokerName,
      brokerCompany,
      brokerPhone,
      brokerEmail,
